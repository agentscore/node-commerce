...
| [`compute-first-merchant.ts`](./compute-first-merchant.ts) | Pay-per-result variable cost (LLM, transcode, per-token/byte) | The probe leg runs the work server-side, caches the result by body content-hash, and emits a 402 at the **exact** computed price; the retry pays that price and gets the cached result. Exact-mode rails only (x402-exact Base, plus Tempo/Solana/Stripe SPT via a `composeMppx` callback) — deliberately scoped out of x402-upto (Permit2) and Settlement-Overrides. Uses `computeFirstCheckout` + `createQuoteCache`; pairs with `rateLimitHono` since the probe leg runs work pre-payment. |
