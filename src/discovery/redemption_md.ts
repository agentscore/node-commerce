/**
 * Standard `/redemption.md` template for merchants offering printed-mailer
 * redemption codes.
 *
 * Renders the canonical cold-start bootstrap section + TL;DR + recovery table
 * + body/code rules. Merchants supply only the merchant-specific bits (name,
 * URL, SKU intro, peer-merchant pointer) and the rest comes from this template.
 *
 ...
 *
 * `skuIntro` is one paragraph describing what the code unlocks at this
 * merchant (e.g. "a wine SKU you'll find in /catalog with purchase_mode =
 * redemption_only"). Defaults to a generic placeholder.
 *
 * `peerMerchantPointer` is the optional "Don't have a code?" cross-link at
 * the bottom; a URL or short markdown line pointing at the merchant-name's
...
export function buildRedemptionSkillMd(opts: {
  ...
  const { merchantName, appUrl, skuIntro, peerMerchantPointer } = opts;
  const skuText =
    skuIntro ??
  ...
  const peerSection = peerMerchantPointer
    ? '\n## Don\'t have a code?\n\nThis page is the redemption flow for printed-mailer codes. ' +
      `If you're looking to buy without a code, see: ${peerMerchantPointer}\n`
    : '';

  return `# Redeeming an AgentScore mailer code at ${merchantName}
...
${skuText} The 402 challenge on /purchase tells you the actual settle amount
...
2. \`GET ${appUrl}/catalog\`; find the product whose \`purchase_mode\` is
   \`redemption_only\`. Read its \`purchase_note\` for any product-specific rules.
3. \`POST ${appUrl}/purchase\` with body:
   \`\`\`json
   {
     "product_slug": "<slug>",
...
5. On **402**, the body carries \`accepted_methods\` and \`agent_instructions.how_to_pay\`.
   Settle with \`agentscore-pay pay POST ${appUrl}/purchase --chain <rail> -d '<body>'
