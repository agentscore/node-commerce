...
describe('agentscoreGate middleware — API error', () => {
  ...
  it('returns 503 api_error on fetch rejection when failOpen is false', async () => {
    mockFetchReject();
    const mw = agentscoreGate({ apiKey: API_KEY });
    const req = makeReq(WALLET);
    const { res, status, json } = makeRes();
  ...
  it('returns 503 api_error on non-402 HTTP error when failOpen is false', async () => {
    mockFetchStatus(500);
    const mw = agentscoreGate({ apiKey: API_KEY });
    const req = makeReq(WALLET);
    const { res, status, json } = makeRes();
...
describe('agentscoreGate middleware — edge cases', () => {
  ...
  it('sends User-Agent header matching @agent-score/commerce format with version', async () => {
    mockFetchOk(ALLOW_RESPONSE);
    const mw = agentscoreGate({ apiKey: API_KEY });
...
describe('agentscoreGate middleware — verify_url and operator_verification in response', () => {
  ...
  it('creates a session and returns 403 identity_verification_required when identity missing', async () => {
    mockFetchOk({
      session_id: 'sess_ex1',
      poll_secret: 'ps_ex',
      verify_url: 'https://www.agentscore.com/verify/ex',
  ...
  it('re-mints a verification session from a CACHED fixable denial on the second request', async () => {
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({
        ok: true, status: 200, headers: new Headers({ 'retry-after': '0' }),
        json: vi.fn().mockResolvedValue({ decision: 'deny', decision_reasons: ['kyc_required'], verify_url: 'https://www.agentscore.com/verify/c1' }),
      })
      // 1st request: session mint POST
      .mockResolvedValueOnce({
        ok: true, status: 200, headers: new Headers({ 'retry-after': '0' }),
        json: vi.fn().mockResolvedValue({ session_id: 'sess_1', poll_secret: 'ps_1', verify_url: 'https://www.agentscore.com/verify/s1' }),
      })
      // 2nd request: session mint POST (cached-deny path; no /v1/assess)
      .mockResolvedValueOnce({
        ok: true, status: 200, headers: new Headers({ 'retry-after': '0' }),
        json: vi.fn().mockResolvedValue({ session_id: 'sess_2', poll_secret: 'ps_2', verify_url: 'https://www.agentscore.com/verify/s2' }),
      });
    global.fetch = fetchMock as unknown as typeof fetch;
  ...
  it('exposes the full assess response on reason.data in custom onDenied', async () => {
    const denyWithPolicy = {
      ...
      verify_url: 'https://www.agentscore.com/verify/xyz',
