...
import { agentscoreGate } from '../src/identity/express';
...
const expressSrc = readFileSync(new URL('../src/identity/express.ts', import.meta.url), 'utf-8');
...
describe('source code structure', () => {
  ...
  it('core sends User-Agent header with package version', () => {
    ...
    expect(coreSrc).toContain('@agent-score/commerce@');
  });
...
describe('error response edge cases', () => {
  ...
  it('returns 503 api_error on HTTP 503 when failOpen is false', async () => {
    mockFetchStatus(503);
  ...
  it('returns 503 api_error on HTTP 429 rate limit when failOpen is false', async () => {
    mockFetchStatus(429);
    const mw = agentscoreGate({ apiKey: API_KEY });
    const req = makeReq(WALLET);
    const { res, status, json } = makeRes();
...
describe('verify_url in DenialReason', () => {
  ...
  it('passes verify_url to defaultOnDenied on fresh deny', async () => {
    const denyWithVerifyUrl = {
      ...DENY_RESPONSE,
      verify_url: 'https://agentscore.com/verify/test123',
    };
  ...
  it('passes verify_url to custom onDenied on fresh deny', async () => {
    const denyWithVerifyUrl = {
      ...DENY_RESPONSE,
      verify_url: 'https://agentscore.com/verify/custom123',
    };
    mockFetchOk(denyWithVerifyUrl);
  ...
  it('passes verify_url through cache on repeated deny', async () => {
    const denyWithVerifyUrl = {
      ...DENY_RESPONSE,
      verify_url: 'https://agentscore.com/verify/cached123',
    };
    mockFetchOk(denyWithVerifyUrl);
    const onDenied = vi.fn();
